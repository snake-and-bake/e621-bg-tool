...
rainbow_table = desktop_wallpaper_directories / ".rainbowtable"
...
def choose_genders():
    ...
    print('Choose Solo or Multiple Characters:\n1) Solo\n2) Multiple\nNone/Any) Randomly Choose')
    count_choice = input('> ')
    count = 0
...
def write_entry(image_id):
    try:
        with open(rainbow_table, 'a+') as file:
            file.write(str(image_id)+"\n")
        return True
    except:
        raise Exception('Writing to rainbow table failed!!!')
...
def main():
    ...
    want_tutorial = input('Do you want the full tutorial, or just the tips? (;3)\n(F)ull tutorial\n(T)ips\n(None/Any) Neither\n> ')
    if len(want_tutorial) == 0:
        want_tutorial = 'N'
    if want_tutorial[0].upper() in ['F', 'T']:
        run_tutorial(want_tutorial[0].upper())
    if not rainbow_table.exists():
        with open(rainbow_table, 'x') as file:
            file.write(f'Initialized on {date.today()}\n')
    print('Starting...')
    repeat = True
    while repeat:
        ...
        print(f'Query: {query}\nSearching for posts...')
        try:
            grab_posts = lambda x: x.posts.search(query, limit=RETURN_LIMIT, ignore_pagination=True)
            ...
            while len(posts) < count:
                ...
                posts = grab_posts(api)
                print(f'Pulled {len(posts)} images!')
                if len(posts) > 0 and standards <= INITIAL_STANDARDS * allowable_drop_in_standards:
                    print("There are posts, but not enough posts to meet the requested amount. Adjusting count...")
                    count = len(posts)
                    break
        except:
